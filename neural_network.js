class NeuralNetwork {
  ...
  copy() {
    const n = new NeuralNetwork(
      this.inodes, this.hnodes, this.onodes, this.lr);
    n.wIH = math.clone(this.wIH);
    n.wHO = math.clone(this.wHO);
    return n;
  }
  ...
  mutate(rate) {
    ...
    function mutatem(value, index, m) {
      // console.log("mm");
      if (random(1) < rate) {
        // return randomGaussian(0, pow(m.size[1], -0.5));
        return value + random(-0.1, 0.1);
      } else {
        return value;
      }
    }
    this.wIH = math.map(this.wIH, mutatem);
    this.wHO = math.map(this.wHO, mutatem);
  }
